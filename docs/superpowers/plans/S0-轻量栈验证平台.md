...
class MalformedOutputError(Exception): pass  # 模型返回 malformed JSON/结构
class EmptyResponseError(Exception): pass     # 模型返回空响应
class InvalidStructureError(Exception): pass  # 幻觉出非法结构（如图片无 URL）
class ModelRefusalError(Exception): pass      # 模型拒绝生成（合规拒绝）
...
```python
async def _generate_single_asset(task_id: str, page_index: int) -> dict:
    # stub：真实实现在此调用图片生成 API，返回 asset 字段
    import hashlib
    ...
    return {"task_id": task_id, "page_index": page_index, "hash": h,
            ...
            "model_version": "dall-e-3", "is_illustration": False}
...
async def effective_duration(session, review_session: ReviewSession) -> float:
    ...
    if review_session.auto_suspended_at is not None:
        # auto_suspended_at 记录最后一次挂起时刻；挂起至完成之间的时长需扣除。
        # 更精确的多段挂起由 review_actions 的 view_durations 累加替代，这里做单段近似。
...
async def detect_time_inconsistency(task_id: str, view_duration_sum: float):
    ...
    async with SessionLocal() as session:
        result = await session.execute(
            select(ReviewSession).where(ReviewSession.task_id == task_id,
